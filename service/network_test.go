...
func TestGetInterfaces(t *testing.T) {
	...
	for _, iface := range interfaces {
		t.Run(iface.Name, func(t *testing.T) {
			...
			// 验证MAC地址格式
			if iface.Hardware.MACAddress != "" {
				macPattern := `^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})package service
...
func TestGetInterface(t *testing.T) {
	// 创建模拟服务
	mock := newMockNetworkService()

	// 测试获取存在的网卡
	iface, exists := mock.interfaces["eth0"]
	if !exists {
		t.Error("未找到预期的测试网卡")
	}
	...
	// 验证IPv6配置
	if iface.IPv6Config.IP != "2001:db8::1" {
		t.Errorf("IPv6地址不匹配: 期望 2001:db8::1, 得到 %s", iface.IPv6Config.IP)
	}
}
